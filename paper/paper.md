...
I tested one property that a generator has to get right before any of the others
matter: how long a job takes. If domain reasoning alone can predict that, a
taxonomy is enough to build a generator. If it cannot, the behaviour has to come
from the client's own records, and knowing which case you are in is worth money.
...
I fixed that in numbers before running anything. Rules written without sight of
the target data should reach an AUC between 0.60 and 0.72, and anything below
0.55 would mean the reasoning adds nothing. The null I test against is that the
rules carry no information about which kind of work runs slow, so that any score
they reach is available from carving up the same taxonomy at random.
...
This ladder tests my first recorded hypothesis, fixed in `HYPOTHESIS.md` before I
